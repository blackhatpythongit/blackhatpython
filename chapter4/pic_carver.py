...
def get_http_headers(http_payload):
	try:
		# split the headers off if is HTTP traffic
...
def face_detect(path, file_name):
	img = cv2.imread(path)
	cascade = cv2.CascadeClassfier("haarcascade_frontalface_alt.xml")
	...
	cv2.imwrite("%s/%s-%s" % (faces_diectory, pacp_file, file_name), img)
	return True
